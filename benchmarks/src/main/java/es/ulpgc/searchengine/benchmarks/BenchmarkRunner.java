...
public class BenchmarkRunner {
    ...
    private BenchmarkResult runSinglePipeline(int bookId) {
        ...
        try {
            ...
            int ic = HttpHelper.post(ingestionUrl + "/ingest/" + bookId);
            r.ingestionTime = Duration.between(ti, Instant.now()).toMillis();
            if (ic != 200) throw new RuntimeException("Ingestion HTTP " + ic);

            // ── Indexing ───────────────────────────────────────────────────
            Instant idx = Instant.now();
            int ixc = HttpHelper.post(indexingUrl + "/index/update/" + bookId);
            r.indexingTime = Duration.between(idx, Instant.now()).toMillis();
            if (ixc != 200) throw new RuntimeException("Indexing HTTP " + ixc);
