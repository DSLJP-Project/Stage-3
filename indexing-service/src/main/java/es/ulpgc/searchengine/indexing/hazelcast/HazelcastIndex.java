...
public class HazelcastIndex {
    ...
    private final IMap<String, Long> reindexState;
    ...
    public HazelcastIndex() {
        ...
        reindexState = hz.getMap("reindex_state");
    ...
    public void deleteBook(int bookId) {
        ...
        try {
            for (int shard = 0; shard < 3; shard++) {
                MultiMap<String, Integer> map = hz.getMultiMap("inverted_index_" + shard);
                for (String term : new HashSet<>(map.keySet())) map.remove(term, bookId);
            }
        } finally {
            locks.unlock(lockKey);
        }
    ...
    public void markReindexReady(long generation) {
        reindexState.set("ready_generation", generation);
    }

    public boolean awaitReindexReady(long generation, long timeoutMillis) {
        long deadline = System.currentTimeMillis() + timeoutMillis;
        while (System.currentTimeMillis() < deadline) {
            Long current = reindexState.get("ready_generation");
            if (current != null && current >= generation) return true;
            try { Thread.sleep(50); }
